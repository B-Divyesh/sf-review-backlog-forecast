...
The sample supports every claim check: both CSV shapes, Anki count-to-CSV steps, raw-import privacy, input and plan persistence, JSON backup round-trip, daily cards and minutes, schedule CSV export, local clearing, offline reload, and app updates. Each check starts in a fresh browser context or Playwright test page. The exact commands are in `.factory/claims.json`.
