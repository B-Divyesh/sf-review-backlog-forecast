...
Try the complete sample at `/demo/`, or use **Try it with sample data** on the first screen. `?demo=1` also opens the isolated sample. The demo opens a 320-card overdue queue in its own browser database. It never reads or changes real plans.
...
`npm run test:e2e` builds the production app before starting Playwright. The suite checks desktop and 390px mobile layouts, keyboard use, accessibility, privacy, persistence, and exports. It also checks offline reopening, updates, page details, 404 behavior, touch targets, and content movement while loading. Every visitor-facing product claim and its exact test command is listed in `.factory/claims.json`. The isolated test data is documented in `.factory/demo.md`.

## Privacy and architecture

Vite and TypeScript produce an installable offline web app. It loads no third-party fonts, scripts, analytics, or advertising. It does not fetch images from another service. No account or payment step is required. Inputs and the last chosen plan stay in this browser’s local database. After the first visit, the browser’s offline cache keeps the app available without a connection. See [the privacy page](https://review-backlog-forecast.sociobot.in/privacy/) and [the terms](https://review-backlog-forecast.sociobot.in/terms/).
